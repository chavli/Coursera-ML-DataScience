...
p = brewer.pal(3, "Set3")
plot(gss$consci, xlab="Opinion", ylab="# of Responses", main="Distribution of Responses for CONSCI", col=p)
summary(gss$consci)
```

```{r fig.height=5, fig.width=10, fig.align='center'}
p = brewer.pal(3, "Accent")
plot(gss$natspac, xlab="Opinion", ylab="# of Responses", main="Distribution of Responses for NATSPAC", col=p)
summary(gss$natspac)
```
...
plot(gss$natspac ~ gss$consci, xlab="CONSCI",ylab="NATSPAC", main="Joint Relative Frequency of NATSPAC and CONSCI Responses", col=p)
...
```{r echo=FALSE}
clean_gss = gss[, c("natspac", "consci")]
clean_gss = na.omit(clean_gss)
```
...
```{r echo=FALSE}
observed_t = addmargins(table(gss$natspac, gss$consci))
observed_t
```
...
```{r echo=FALSE}
  expected_t = table(gss$natspac, gss$consci)
  expected_t[c(1, 4, 7)] = round(observed_t[c(4, 8, 12)] * (observed_t[13] / observed_t[16]))
  expected_t[c(2, 5, 8)] = round(observed_t[c(4, 8, 12)] * (observed_t[14] / observed_t[16]))
  expected_t[c(3, 6, 9)] = round(observed_t[c(4, 8, 12)] * (observed_t[15] / observed_t[16]))
  expected_t
```
...
```{r echo=FALSE}
  full_expected_t = addmargins(expected_t)
  chi = 0
  for(i in c(1, 2, 3, 5, 6, 7, 9, 10, 11)){
    chi = chi + ((observed_t[i] - full_expected_t[i])^2 / full_expected_t[i])
  }
```
$$
...
```{r fig.height=5, fig.width=10, fig.align='center'}
  plot(dchisq(seq(0, 600, .5), df=4), type='l', col='blue', ylab='probability', xlab='chi-squared statistic', main='Chi Squared Distribution (df=4)')
